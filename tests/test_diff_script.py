...
class TestPdfVisualDiff(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if not os.path.exists("tests/test_pdfs"):
            subprocess.run(["python3", "tests/create_test_pdfs.py"], capture_output=True, text=True)
    ...
    def test_identical_pdfs(self):
        ...
        result = subprocess.run(["python3", "pdf_visual_diff.py", pdf1, pdf2, "--output", output_dir], capture_output=True, text=True)
